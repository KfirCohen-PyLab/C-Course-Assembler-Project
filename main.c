...
/*
    Welcome to My assembler program.
...
/* Defined global variables here because they are defined as extern,
...
int main(int argc, char *argv[]){
	...
	/* The macro spreading phase */
	for(i = 1; i < argc; i++){
		
		struct  Macro* Mtail= NULL;
		struct  Macro* Mhead = NULL;
		
		Mhead = (struct Macro*)malloc(sizeof(struct Macro)); /* linked list of macros */
    ...
    for(i = 1; i < argc; i++)
    {
        input_filename = create_file_name(argv[i], FILE_INPUT_MACRO); /* Appending .am to filename */
